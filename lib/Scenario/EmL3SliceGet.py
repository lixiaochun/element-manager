...
class EmL3SliceGet(EmSeparateScenario.EmScenario):
    '''
    ...
    def _gen_sub_thread(
        ...
        GlobalModule.EM_LOGGER.info(
            "104001 Scenario:L3SliceGet Device:%s start", device_name)
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L3SliceGet Device:%s NG:13(Processing failure(temporary))",
                device_name)
            GlobalModule.EM_LOGGER.info(
                "104002 Scenario:L3SliceGet Device:%s end", device_name)
            return
        ...
        if is_comdriver_result == GlobalModule.COM_COMPARE_UNMATCH:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L3SliceGet Device:%s NG:11(Processing failure（Consistent NG）)",
                device_name)
            ...
            GlobalModule.EM_LOGGER.debug("compare_to_db_info NG")
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L3SliceGet Device:%s NG:12(Processing failure（No retention information）)",
                device_name)
        ...
        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L3SliceGet Device:%s NG:14(Processing failure(Other))",
                device_name)
        ...
        is_comdriver_result = self.com_driver_list[
            device_name].compare_to_device_setting(
                device_name, "l3-slice", order_type, return_device_signal)
