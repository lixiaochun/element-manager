...
class EmInternalLagDelete(EmSeparateScenario.EmScenario):


    @decorater_log
    def __init__(self):

        super(EmInternalLagDelete, self).__init__()
    ...
    def _gen_sub_thread(
        ...
        GlobalModule.EM_LOGGER.info(
            "104001 Scenario:InternalLagDelete Device:%s start", device_name)
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:InternalLagDelete Device:%s NG:13(処理失敗(一時的))",
                device_name)
            GlobalModule.EM_LOGGER.info(
                "104002 Scenario:InternalLagDelete Device:%s end", device_name)
            return
        ...
        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:InternalLagDelete Device:%s NG:14(処理失敗(その他))",
                device_name)
        ...
        if is_comdriver_result == GlobalModule.COM_DELETE_VALICHECK_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:InternalLagDelete Device:%s \
