...
class EmCeLagMerge(EmSeparateScenario.EmScenario):
    ...
    def _gen_sub_thread(
        ...
        '''
        Conduct adding LAG control for CE for each device.
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:CeLagMerge Device:%s NG:13(Processing failure (temporary))",
                device_name)
            GlobalModule.EM_LOGGER.info(
        ...
        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:CeLagMerge Device:%s NG:14(Processing failure (Other))",
                device_name)
        ...
        if is_comdriver_result == GlobalModule.COM_UPDATE_VALICHECK_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:CeLagMerge Device:%s NG:9(Processing failure(validation check NG))",
                device_name)
