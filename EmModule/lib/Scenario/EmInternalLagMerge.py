...
class EmInternalLagMerge(EmSeparateScenario.EmScenario):
    ...
    def __init__(self):
        ...
        super(EmInternalLagMerge, self).__init__()
    ...
    def _gen_sub_thread(
        ...
        GlobalModule.EM_LOGGER.info(
            "104001 Scenario:InternalLagMerge Device:%s start", device_name)
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:InternalLagMerge Device:%s NG:13(処理失敗(一時的))",
                device_name)
            GlobalModule.EM_LOGGER.info(
                "104002 Scenario:InternalLagMerge Device:%s end", device_name)
            return
        ...
        is_comdriver_result = self.com_driver_list[device_name].connect_device(
            device_name, "internal-lag", order_type, json_message)

        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            GlobalModule.EM_LOGGER.debug("connect_device 装置接続NG")
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:InternalLagMerge Device:%s NG:14(処理失敗(その他))",
                device_name)
        ...
        if is_comdriver_result == GlobalModule.COM_UPDATE_VALICHECK_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:InternalLagMerge Device:%s \
                NG:9(処理失敗(バリチェックNG))", device_name)
        ...
        is_comdriver_result = self.com_driver_list[
            device_name].reserve_device_setting(
                device_name, "internal-lag", order_type)
        ...
        is_comdriver_result = self.com_driver_list[device_name].write_em_info(
            device_name, "internal-lag", order_type, device_message)
    ...
    def __creating_json(self, device_message):
        ...
        vpn_type = self._find_xml_node(xml_elm, ns_internal_lag + "vpn-type")

        if vpn_type is not None:
            device_json_message[
                "device"].update({"vpn-type": vpn_type.text})

        device_elm = self._find_xml_node(xml_elm, ns_internal_lag + "name")
        device_json_message["device"]["name"] = device_elm.text

        itnal_tag = ns_internal_lag + "internal-lag-interface"
            ...
            itnal_int_tag = ns_internal_lag + "internal-interface"
                interface_name = itnal_int.find(ns_internal_lag + "name").text
            ...
            internal_lag_message["name"] = \
                itnal_lag.find(ns_internal_lag + "name").text
            internal_lag_message["minimum-links"] = \
                itnal_lag.find(ns_internal_lag + "minimum-links").text
            internal_lag_message["link-speed"] = \
                itnal_lag.find(ns_internal_lag + "link-speed").text
            internal_lag_message["address"] = \
                itnal_lag.find(ns_internal_lag + "address").text
            internal_lag_message["prefix"] = \
                itnal_lag.find(ns_internal_lag + "prefix").text

            device_json_message["device"][
                "internal-lag"].append(internal_lag_message)

        device_json_message["device"]["internal-lag_value"] = \
            len(device_json_message["device"]["internal-lag"])
