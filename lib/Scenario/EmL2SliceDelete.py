...
# Filename: EmL2SliceDelete.py
'''
Individual scenario for deleting L2 slice.  
'''
import threading
import json
...
class EmL2SliceDelete(EmSeparateScenario.EmScenario):
    '''
    Class for L2 slice deletion. 
    '''
    ...
    def _gen_sub_thread(
        ...
        '''
        Conduct L2 slice deletion control for each device. 
        ...
        GlobalModule.EM_LOGGER.info(
            "104001 Scenario:L2SliceDelete Device:%s start", device_name)
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L2SliceDelete Device:%s NG:13(Processing failure (temporary))",
                device_name)
            GlobalModule.EM_LOGGER.info(
                "104002 Scenario:L2SliceDelete Device:%s end", device_name)
            return
        ...
        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L2SliceDelete Device:%s NG:14(Processing failure (Other))",
                device_name)
        ...
        if is_comdriver_result == GlobalModule.COM_DELETE_VALICHECK_NG:
            ...
            if force is not True:
                ...
                if is_comdriver_result is False:
                    ...
                    GlobalModule.EM_LOGGER.warning(
                        "204004 Scenario:L2SliceDelete Device:%s NG:13(Processing failure)",
                        device_name)
                    ...
                    GlobalModule.EM_LOGGER.info(
                        "104002 Scenario:L2SliceDelete Device:%s end",
                        device_name)
                    return
                ...
                if is_comdriver_result is False:
                    ...
                    GlobalModule.EM_LOGGER.warning(
                        "204004 Scenario:L2SliceDelete Device:%s" +
