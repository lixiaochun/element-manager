...
class EmCeLagMerge(EmSeparateScenario.EmScenario):
    ...
    def _gen_sub_thread(
        ...
        GlobalModule.EM_LOGGER.info(
            "104001 Scenario:CeLagMerge Device:%s start", device_name)
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:CeLagMerge Device:%s NG:13(処理失敗(一時的))",
                device_name)
            GlobalModule.EM_LOGGER.info(
                "104002 Scenario:CeLagMerge Device:%s end", device_name)
            return
        ...
        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:CeLagMerge Device:%s NG:14(処理失敗(その他))",
                device_name)
        ...
        if is_comdriver_result == GlobalModule.COM_UPDATE_VALICHECK_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:CeLagMerge Device:%s NG:9(処理失敗(バリチェックNG))",
                device_name)
    ...
    def __creating_json(self, device_message):
        ...
        ce_tag = ns_ce_lag + "ce-lag-interface"
            ...
            leaf_tag = ns_ce_lag + "leaf-interface"
                interface_name = leaf_int.find(ns_ce_lag + "name").text
            ...
            ce_lag_message["link-speed"] = \
                ce_lag.find(ns_ce_lag + "link-speed").text
