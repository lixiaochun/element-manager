    ...
    def _gen_sub_thread(
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L3SliceGet Device:%s NG:13(処理失敗(一時的))",
                device_name)
            GlobalModule.EM_LOGGER.info(
        ...
        if is_comdriver_result == GlobalModule.COM_COMPARE_UNMATCH:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L3SliceGet Device:%s NG:11(処理失敗（整合NG）)",
                device_name)
            ...
            GlobalModule.EM_LOGGER.debug("compare_to_db_info NG")
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L3SliceGet Device:%s NG:12(処理失敗（保持情報なし）)",
                device_name)
        ...
        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L3SliceGet Device:%s NG:14(処理失敗(その他))",
                device_name)
