...
class EmInternalLinkMerge(EmSeparateScenario.EmScenario):
    ...
    def _gen_sub_thread(
        ...
        GlobalModule.EM_LOGGER.info(
            "104001 Scenario:InternalLinkMerge Device:%s start", device_name)
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:InternalLinkMerge Device:%s NG:13(Processing failure (temporary))",
                device_name)
            GlobalModule.EM_LOGGER.info(
                "104002 Scenario:InternalLinkMerge Device:%s end", device_name)
            return
        ...
        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:InternalLinkMerge Device:%s NG:14(Processing failure (Other))",
                device_name)
