    ...
    def _gen_sub_thread(
        ...
        if is_db_result is False:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L2SliceDelete Device:%s NG:13(処理失敗(一時的))",
                device_name)
            GlobalModule.EM_LOGGER.info(
        ...
        if is_comdriver_result == GlobalModule.COM_CONNECT_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L2SliceDelete Device:%s NG:14(処理失敗(その他))",
                device_name)
        ...
        if is_comdriver_result == GlobalModule.COM_DELETE_VALICHECK_NG:
            ...
            GlobalModule.EM_LOGGER.warning(
                "204004 Scenario:L2SliceDelete Device:%s NG:9(処理失敗(バリチェックNG))",
                device_name)
            ...
            if force is not True:
                ...
                if is_comdriver_result is False:
                    ...
                    GlobalModule.EM_LOGGER.warning(
                        "204004 Scenario:L2SliceDelete Device:%s NG:13(処理失敗)",
                        device_name)
