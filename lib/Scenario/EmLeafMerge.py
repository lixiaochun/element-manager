...
class EmLeafMerge(EmDeviceMerge):
    '''
    ...
    def _creating_json(self, device_message):
        ...
        self._gen_json_vpn(
            device_json_message, xml_elm, self._xml_ns, vpn="l2-vpn")
    ...
    def _gen_json_equipment(self, json, xml, xml_ns):
        ...
        tmp = equ_elm.find(xml_ns + "q_in_q")
        if tmp is not None:
            equ_json["q-in-q"] = tmp.text
    ...
    def _gen_json_vpn(self, json, xml, xml_ns, vpn="l3-vpn"):
        ...
        if vpn == "l3-vpn" or vpn == "l2-vpn":
            vpn_elm = self._find_xml_node(xml, xml_ns + vpn)
            if vpn_elm is not None:
                device_json_message = {
                    vpn: {
                        "bgp": [],
                        "as": {
                ...
                bgp_elm = self._find_xml_node(vpn_elm, xml_ns + "bgp")
                neighbor_tag = xml_ns + "neighbor"
                for neighbor in bgp_elm.findall(".//" + neighbor_tag):
                    neighbor_addr = neighbor.find(xml_ns + "address").text
                    bgp_message["neighbor"].append(
                        {"address": neighbor_addr})

                bgp_message["community"] = \
                    bgp_elm.find(xml_ns + "community").text

                bgp_message["community-wildcard"] = \
                    bgp_elm.find(xml_ns + "community-wildcard").text

                as_elm = self._find_xml_node(vpn_elm, xml_ns + "as")

                device_json_message[vpn]["as"]["as-number"] = \
                    int(as_elm.find(xml_ns + "as-number").text)
